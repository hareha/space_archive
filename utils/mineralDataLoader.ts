        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            try {
                const mineralData: MineralData = {
                    ...
                    // 산화물은 원본이 g/g (weight fraction) → wt% 변환 (×100)
                    mgo: parseFloat(values[7]) * 100,
                    al2o3: parseFloat(values[8]) * 100,
                    sio2: parseFloat(values[9]) * 100,
                    cao: parseFloat(values[10]) * 100,
                    tio2: parseFloat(values[11]) * 100,
                    feo: parseFloat(values[12]) * 100,
                    // K, Th, U는 이미 ppm 단위
